...
| Indexed static navigation | Yes, for Python, JavaScript, TypeScript, and Vue | Yes | Workflow defined; no checked-in remote run receipt | No | Compiler-grade call resolution and more languages | Import and call views are navigation candidates from indexed metadata and identifier-pattern matching. They do not resolve aliases, dynamic dispatch, overloads, re-exports, or cross-language calls; inspect source before relying on an edge. |
