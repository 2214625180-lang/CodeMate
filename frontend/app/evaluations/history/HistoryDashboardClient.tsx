    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-slate-950">Evaluation History</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-600">
            Track benchmark quality, latency, gate status, and failed cases across evaluation runs.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          {selectedDataset ? (
            <div className="rounded-md border border-border bg-slate-50 px-3 py-2 text-sm text-slate-600">
              {selectedDataset.task_type} · {selectedDataset.cases_json.length} cases · baseline{" "}
      ...
      {isLoadingHistory ? (
        ...
        <section className="rounded-lg border border-border bg-white p-5 text-sm text-slate-500">
          Select a benchmark dataset to view history.
        </section>
      )}
    </div>
  );
  ...
  return (
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <SummaryCard
        ...
        detail={latestDelta === null ? "No previous completed run" : `${formatDelta(latestDelta)} vs previous`}
      />
      <SummaryCard
        label="Completed Runs"
        value={`${completedCount}/${runCount}`}
        detail={`${numberValue(summary.failed_run_count) ?? 0} failed · ${numberValue(summary.running_count) ?? 0} running`}
      />
      <SummaryCard
        label="Avg Latency"
      ...
      <SummaryCard
        ...
        detail={history.dataset.task_type === "fix" ? "Fix-agent runs" : "Retrieval runs"}
      />
    </section>
  );
}
  ...
  return (
    <section className="rounded-lg border border-border bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-950">
            {metricLabel(history.primary_metric_name)} Trend
          </h2>
          <p className="mt-1 text-sm text-slate-500">
            {completedRuns.length} completed runs · newest on the right
          </p>
        </div>
      ...
      {completedRuns.length === 0 ? (
        <p className="mt-8 text-sm text-slate-500">No completed runs with metric data yet.</p>
      ) : (
        <div className="mt-6">
          <TrendSvg runs={completedRuns} />
          <div className="mt-4 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
            <span>Oldest: {shortRunLabel(completedRuns[0])}</span>
            <span className="sm:text-right">
              Newest: {shortRunLabel(completedRuns[completedRuns.length - 1])}
            </span>
          </div>
        </div>
      )}
...
function TrendSvg({ runs }: { runs: EvaluationHistoryRun[] }) {
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Primary metric trend"
    >
...
function GateStatusPanel({ history }: { history: EvaluationHistory }) {
  const statuses: Array<{
    ...
    { key: "passed", label: "Passed", className: "bg-emerald-50 text-emerald-700" },
    { key: "failed", label: "Failed", className: "bg-red-50 text-red-700" },
    { key: "inconclusive", label: "Inconclusive", className: "bg-amber-50 text-amber-700" },
    { key: "not_evaluated", label: "Not evaluated", className: "bg-slate-100 text-slate-600" }
  ];

  return (
    <section className="rounded-lg border border-border bg-white p-5">
      <h2 className="text-lg font-semibold text-slate-950">Gate Status</h2>
      ...
      <div className="mt-6 border-t border-border pt-4">
        <p className="text-sm font-medium text-slate-700">Baseline</p>
        <p className="mt-1 break-all text-sm text-slate-500">
          {history.baseline_run_id ?? "No baseline configured"}
        </p>
      </div>
    </section>
  );
...
function RecentHistoryRuns({ history }: { history: EvaluationHistory }) {
  return (
    <section className="rounded-lg border border-border bg-white p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-slate-950">Runs</h2>
        <span className="text-sm text-slate-500">{history.runs.length} recent runs</span>
      </div>

      {history.runs.length === 0 ? (
        <p className="mt-4 text-sm text-slate-500">No evaluation runs for this dataset yet.</p>
      ) : (
...
function renderTrendReportMarkdown(history: EvaluationHistory) {
  ...
  const lines = [
    `# Evaluation Trend Report: ${escapeMarkdown(history.dataset.name)}`,
    "",
    "## Summary",
    "",
    "| Field | Value |",
    "| --- | --- |",
    tableRow("Dataset", `${history.dataset.name} (${history.dataset.id})`),
    tableRow("Task Type", history.dataset.task_type),
    tableRow("Dataset Version", history.dataset.version),
    tableRow("Baseline Run", history.baseline_run_id ?? "not configured"),
    tableRow("Primary Metric", metricLabel(history.primary_metric_name)),
    tableRow("Latest Metric", formatPercent(numberValue(summary.latest_primary_metric))),
    tableRow("Latest Delta", formatDelta(numberValue(summary.latest_primary_metric_delta))),
    tableRow("Completed Runs", `${numberValue(summary.completed_count) ?? 0}/${numberValue(summary.run_count) ?? history.runs.length}`),
    tableRow("Average Latency", formatSeconds(numberValue(summary.avg_latency_sec))),
    tableRow("Average Tool Calls", formatNullableNumber(numberValue(summary.avg_tool_calls))),
    "",
    "## Filters",
    "",
    "| Filter | Value |",
    "| --- | --- |",
    tableRow("Limit", filters.limit ?? "n/a"),
    tableRow("Created After", filters.created_after ?? "all"),
    tableRow("Created Before", filters.created_before ?? "all"),
    tableRow("Run Status", filters.status ?? "all"),
    tableRow("Gate Status", filters.gate_status ?? "all"),
    tableRow("Provider", filters.provider ?? "all"),
    tableRow("Model", filters.model ?? "all"),
    "",
    "## Gate Status Counts",
    "",
